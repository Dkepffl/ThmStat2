...
```{r}
### load data
estimation_df <- read_excel("Datas", sheet=2)
...
```{r}
data_split <- initial_split(cox_df, prop = 0.8, strata = delta)
train <- training(data_split); test <- testing(data_split)

res_data_train <- data_pipeline(train, Train=T)
train_df_list <- res_data_train[-4]; scaling_par <- res_data_train[[4]]
